...
def build_nodes(events, H, W,
    ...
    # print("Lanzando DBSCAN con", len(points), "eventos")
    labels = DBSCAN(
        eps=eps,
        min_samples=min_samples
    ).fit_predict(points)
    
    # print("DBSCAN terminado")

    num_noise = np.sum(labels == -1)

    # print(
    #     f"Ruido: {100 * num_noise / len(labels):.2f}%"
    # )
    ...
    for label in unique_labels:
        ...
        x_mean = (xs_c.mean() / W) * 2 - 1
        y_mean = (ys_c.mean() / H) * 2 - 1
        t_mean = ((ts_c.mean() - t_min) / t_range) * 2 - 1
        ...
        x_std = xs_c.std() / W
        y_std = ys_c.std() / H
        t_std = ts_c.std() / t_range

        num_events_norm = np.log1p(num_events)

        pos_ratio = np.sum(ps_c > 0) / (num_events + 1e-6)
        ...
        xs_sorted = xs_c[order]
        ys_sorted = ys_c[order]
        ts_sorted = ts_c[order]

        A = np.vstack([
            ts_sorted,
            np.ones_like(ts_sorted)
        ]).T

        vx, _ = np.linalg.lstsq(
            A,
            xs_sorted,
            rcond=None
        )[0]

        vy, _ = np.linalg.lstsq(
            A,
            ys_sorted,
            rcond=None
        )[0]

        dt_local = ts_sorted[-1] - ts_sorted[0] + 1e-6
        ...
        xs_n = xs_c / W
        ys_n = ys_c / H
        ts_n = (ts_c - t_min) / t_range

        cov_xy = np.cov(xs_n, ys_n)[0, 1]
        cov_xt = np.cov(xs_n, ts_n)[0, 1]
        cov_yt = np.cov(ys_n, ts_n)[0, 1]
        ...
        hist = hist / (num_events + 1e-6)

        event_rate = np.log1p(
            num_events / dt_local
        )

        dt_cell = dt_local / t_range
...
def generate_nodes():
    # print("Entrando en generate_nodes()")
    NODE_FEAT_DIM = 19
    node_dir = EVENTS_OUT_DIR / "nodesPose8clustering"
    ...
    # print("Carpeta de eventos:", EVENTS_OUT_DIR)
    # print("Número de archivos .h5:", len(event_files))
    ...
    for ev_file in event_files:
        ...
        # print("Eventos cargados:", len(evs))
        ...
        print(
            f"Frame {idx}: {len(events)} eventos -> "
            f"{len(coords)} nodos"
        )
        ...
        print(
            f"Frame {idx}: {len(coords)} nodos, "
            f"{len(edge_index)} aristas"
        )
