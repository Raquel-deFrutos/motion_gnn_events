...
def build_nodes(
    ...
    _, neigh_idx = nbrs.kneighbors(points)

    feats = []
    coords = []

    # usamos un nodo cada stride eventos

    seeds = np.arange(0, len(events), stride)
    ...
    for seed in seeds:
        ...
        cov_xy = np.cov(xs_c / W, ys_c / H)[0, 1]
        ...
        # num_events = len(idxs)
        # num_events = np.log1p(num_events)
...
def generate_nodes():
    ...
    NODE_FEAT_DIM = 18
    node_dir = EVENTS_OUT_DIR / "nodesPoseKNN"
